...
const JWT_SECRET = 'your_super_secret_key'; // แนะนำให้เปลี่ยนเป็นคีย์ลับของคุณ
...
const pool = new Pool({
  ...
  password: '248543', // ‼️ ตรวจสอบให้แน่ใจว่าเป็นรหัสผ่านของคุณ
  port: 5432,
});
...
app.get('/api/invoices', authenticateToken, async (req, res) => { try { const query = `SELECT i.id, i.invoice_date, i.total_amount, i.payment_method, c.first_name, c.last_name FROM invoices i LEFT JOIN customers c ON i.customer_id = c.id ORDER BY i.invoice_date DESC`; const r = await pool.query(query); res.json(r.rows); } catch (e) { res.status(500).send(e.message) } });
app.get('/api/invoices/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    let invoiceData = {};
    const invoiceResult = await pool.query(`SELECT i.*, c.first_name, c.last_name, c.phone, c.address FROM invoices i LEFT JOIN customers c ON i.customer_id = c.id WHERE i.id = $1`, [id]);
    
    if (invoiceResult.rows.length === 0) {
      return res.status(404).json({ message: 'Invoice not found' });
    }
    invoiceData = invoiceResult.rows[0];

    // 👇 แก้ไข query นี้ให้ดึง unit_name มาด้วย
    const itemsResult = await pool.query(
      `SELECT ii.quantity, ii.price_per_unit, p.name, p.brand, p.unit_name 
...
app.delete('/api/invoices/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // 3. ลบรายการสินค้าในบิล (invoice_items)
    await client.query('DELETE FROM invoice_items WHERE invoice_id = $1', [id]);

    // 4. ลบตัวบิลหลัก (invoices)
...
app.get('/api/backup/database', authenticateToken, (req, res) => {
  ...
  // 👇 สร้าง environment variables สำหรับ process นี้โดยเฉพาะ
  const env = {
    ...process.env,
    PGPASSWORD: dbConfig.password,
  };

  // 👇 ส่ง env เข้าไปใน exec
